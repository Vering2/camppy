...
//			CampRegDTO campRegDTO=campRegService.getCampReg2(campname);
			...
			dispatcher = request.getRequestDispatcher("reservepage/reserveDetail/reserve_detail.jsp");
...
//			request.setAttribute("pageList", PageList);
				...
				dispatcher = request.getRequestDispatcher("reservepage/mypageReserve/mypage_reserve.jsp");
				dispatcher.forward(request, response);
			}
		}
...
//			request.setAttribute("pageList", PageList);
				...
				dispatcher = request.getRequestDispatcher("reservepage/mypageReserve/admin_reserve.jsp");
				dispatcher.forward(request, response);
			}
		}
...
//			String id="ljy";
			...
			if (reserveDetailDTO != null) {
				// 아이디 있음 => 아이디 중복
				dispatcher = request.getRequestDispatcher("reservepage/mypageReserveDetail/DetailList.jsp");
				dispatcher.forward(request, response);
			} else {
				// 아이디 없음 => 아이디 사용가능
				response.setContentType("text/html; charset=UTF-8");
