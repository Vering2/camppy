...
<body>
			...
			<div>
				<table class="table" s>
					...
					<tr>
						...
						<td><%=rdto.getSprice() / rdto.getCamp_price()%>박 <script
 ...
 %> <a
							href="reviewInsert.rv?res_id=<%=rdto.getRes_id()%>&camp_id=<%=rdto.getCamp_id()%>"
							...
							class="btn btn-outline-success" role="button">작성</a> <%
 }
 } else {
 %> <a class="btn btn-outline-success">완료</a>
							<%
							}
							%>
						</td>
						...
						<td><button type="button" class="btn btn-outline-secondary"
								onclick="location.href='MyReservePro.re?res_id=<%=rdto.getRes_id()%>'">가능</button></td>
					</tr>
					<%
					} else {
					%>
					<td><div class="btn btn-outline-secondary">불가</div></td>
					</tr>
					<%
					}
					%>
			...
			%>
			<a href="mypage_reserve.re?pageNum=<%=startPage - pageBlock%>">[10페이지
				이전]</a>
			<%
			}
			%>
			...
			%>
			<a href="mypage_DetailList?pageNum=<%=startPage + pageBlock%>">[10페이지
				다음]</a>
			<%
			}
			%>
