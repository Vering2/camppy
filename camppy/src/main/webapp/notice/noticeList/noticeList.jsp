...
<body>
	...
	<div class="noticeListList2"
		onclick="location.href='content.no?notice_id=<%=noticeDTO.getNotice_id()%>'">
		...
		<div class="noticeListDate2">
			<div class="noticeList2023"><%=format.format(noticeDTO.getCreate_date())%></div>
		</div>
	...
	<div id="pageControl" class="pagination">
		...
		%>
		<a
			href="list.no?pageNum=<%=noticePageDTO.getStartPage() - noticePageDTO.getPageBlock()%>">Prev</a>
		<%
		}
		%>
		...
		%>
		<a
			href="list.no?pageNum=<%=noticePageDTO.getStartPage() + noticePageDTO.getPageBlock()%>">Next</a>
		<%
		}
		%>
