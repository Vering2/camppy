...
<body>
	...
	<div class="questionListList2"
		onclick="location.href='content.qu?inquiry_id=<%=questionDTO.getInquiry_id()%>'">
		...
		<div class="questionListDate2">
			<div class="questionList2023"><%=format.format(questionDTO.getCreate_date())%></div>
		</div>
	...
	<input type="button" value="글쓰기" class="writebtn"
		onclick="location.href='write.qu'">
	...
	<div id="pageControl" class="pagination">
		...
		%>
		<a
			href="list.qu?pageNum=<%=questionPageDTO.getStartPage() - questionPageDTO.getPageBlock()%>">Prev</a>
		<%
		}
		%>
		...
		%>
		<a
			href="list.qu?pageNum=<%=questionPageDTO.getStartPage() + questionPageDTO.getPageBlock()%>">Next</a>
		<%
		}
		%>
