...
<%
List<CampRegDTO> campregList = (List<CampRegDTO>) request.getAttribute("campregList");
...
<body>
	...
	<form action="listSearch.camp" method="post"
		class="mainpage__search-box-frame">
		<div class="mainpage__search-box">
            ...
            <div class="mainpage__3">상세검색</div>
				...
				%>
				<div class="mainpage__options-13">
					<%=keywordsearch%>
					검색 결과
				</div>
				<%
	...
	<div class="mainpage">
			...
			<div class="mainpage__popular-info">
				<table>
					<tr>
						...
						<td>
							<button type="button"
								onclick="location.href='detail.de?campId=<%=campregDTO.getCampid()%>'">
								<img class="mainpage__forest-7"
									src="campimg/<%=campregDTO.getCampimg()%>" /><br>
								<div class="mainpage__4"><%=campregDTO.getCampname()%></div>
								<br>
								<div class="mainpage__4">
									주소 :
									<%=campregDTO.getCampaddr()%></div>
								<br>
								<div class="mainpage__4">
									연락처 :
									<%=campregDTO.getTel()%></div>
							</button>
		...
		<div class="mainpage__list">
			...
			%>
			<a
				href="listSearch.camp?pageNum=<%=pageDTO.getStartPage() - pageDTO.getPageBlock()%>&keywordsearch=<%=pageDTO.getSearch()%>&sido=<%=pageDTO.getSido()%>&gungu=<%=pageDTO.getGungu()%>">Prev</a>
			<%
			}
			...
			%>
			<a
				href="listSearch.camp?pageNum=<%=i%>&keywordsearch=<%=pageDTO.getSearch()%>&sido=<%=pageDTO.getSido()%>&gungu=<%=pageDTO.getGungu()%>"><%=i%></a>
			<%
			}
			...
			%>
			<a
				href="listSearch.camp?pageNum=<%=pageDTO.getStartPage() + pageDTO.getPageBlock()%>&keywordsearch=<%=pageDTO.getSearch()%>&sido=<%=pageDTO.getSido()%>&gungu=<%=pageDTO.getGungu()%>">Next</a>
			<%
}%>
